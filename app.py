...
def create_venue_submission():
    ...
    if form.validate():
        error = False
        if phone == "":
            flash('Phone number max length is 12 characters', 'error')
            return render_template('forms/new_venue.html', form=form)
        if image_link == "":
        ...
        if seeking_talent == "y":
            ...
            if seeking_description == "":
                flash('Please enter a description for the talent you are seeking.', 'error')
                return render_template('forms/new_venue.html', form=form)
        else:
            seeking_talent = False
        ...
        flash('Venue ' + venue_name + ' could not be listed due to Invalid data!', 'error')
        flash(form.errors)
        return render_template('forms/new_venue.html', form=form)
...
def create_artist_submission():
    ...
    if form.validate():
        error = False
        if phone == "":
            flash('Phone number has max length of 12 characters.', 'error')
            return render_template('forms/new_artist.html', form=form)
        if image_link == "":
            flash('Please provide a valid URL for the image link.', 'error')
            return render_template('forms/new_artist.html', form=form)
        ...
        if seeking_venue == "y":
            seeking_venue = True
            if seeking_description == "":
                flash('Please the desired venue description.', 'error')
                return render_template('forms/new_artist.html', form=form)
        else:
            seeking_venue = False
    ...
    if form.validate():
        ...
        flash('Artist ' + artist_name + ' could not be listed due to Invalid data!', 'error')
        flash(form.errors)
        return render_template('forms/new_artist.html', form=form)
